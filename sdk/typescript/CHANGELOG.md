...
- 2ec549e: `success` on `POST /v1/actuals` becomes optional: it is an observation, not a
  status.
...
- b222102: `ActualsRequest` gains the optional `censoring` field the API contract has
  specified all along.
  ...
  - **Omission is the honest default.** The field is optional and never
    defaulted: an absent field stores _unknown_, which is the truth when nothing
...
- d8083f7: Declare the measured `phases` and `assessment` fields that already arrive on
...
- 6c49bb9: docs: stop advertising idempotent replay that does not exist
...
- d34313e: Billing-hardening for the retry/rate-limit path (TypeScript SDK; the Python SDK
  ...
  - **Fail fast when `Retry-After` exceeds the max backoff.** When a 429's
    ...
    and retries — which would fire _before_ the server's stated success time, a
