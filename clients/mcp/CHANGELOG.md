...
- f7a34de: The copy now leads with the measurement, states the abstention as an answer, and discloses what leaves the machine.

  Three user-facing surfaces change; nothing on the wire does.

  - **The `estimate` tool description.** It promised a token range unconditionally, so a model told it would get a number treats an abstention as a failure — and is then under pressure to re-ask with a rephrased query, which bills a second estimate. The returns sentence now names "no range at all when there is no firm basis to forecast this particular task, which is an honest answer, not an error". The lead also says the stored estimate is what lets the same run's realized spend be _measured_ rather than merely _recorded_ — the forecast is a probability; the measurement is a count. The final sentence (the live-usage disclaimer) is byte-identical.

  - **The void message.** It led with what the product cannot do and carried `(out of domain)` — an internal scenario label — on a public string that reaches users directly. It now reads "No forecast for this task — Budgetary has no firm basis to judge one like it, and won't guess. / This estimate wasn't billed. Proceed on your own judgment — an abstention is an answer, not an error." Per query, never a rate. "This estimate wasn't billed." is unchanged. Everything appended beneath it, and the blank-line seam above that, are untouched.

  - **A one-time data disclosure.** Nothing at runtime previously said what this package transmits; it lived only in a README a user had no reason to open. The first successful `estimate` on a machine now appends a one-time note naming both transmissions and pointing at the README's Privacy section, and `doctor` states a short form of it on every run. It is one call late and says so — by the time anything can render, the task text has already been sent. It is a disclosure, not consent: it asks for nothing, gates nothing, grants nothing, and claims no completeness of its own. It never fires on an error path, and a marker that cannot be written degrades to silence rather than a nag.

  Existing installs see the disclosure once, on their next estimate. The priced render is byte-unchanged.
...
- 2ec549e: Stop reporting a run outcome nobody measured — the four submit paths now send
  `success` only when something observed it.

  `POST /v1/actuals` used to require a boolean `success`: _whether the agent run
  completed its objective_. That is a fact about the **run**, and this client had
  ...
  with absent meaning _the outcome was not observed_ — neither a failure nor a
  ...
  - **`report-actual` now asks a question you can decline.** The old prompt was
    ...
    a failed run. It is now three-state (_yes / no / not sure_), an empty answer
    selects _not sure_, and _not sure_ omits the field. Nothing is pre-selected.
  ...
  - **Absence survives the retry ladder.** The round-trip that persists a failed
    submit's counts and resubmits them from a later session previously _required_
  ...
  they _do_ measure submit exactly as before — an unobserved outcome never costs
...
- b222102: Send how the run ended — from the two paths that can actually observe it, and
  ...
  endpoint — so an actual submitted without it records _unknown_ forever.
  ...
  - **`report-actual` now asks how the run ended** — four categories in plain
    language plus an explicit _not sure / prefer not to say_, which an EMPTY
  ...
  - **The SessionEnd hook path sends nothing, permanently.** Its payload `reason`
    describes how the _session_ ended, not how the _run_ ended — a `/clear`
...
- b68ae8c: Show a recorded run's measured breakdown, instead of throwing it away.
  ...
  answer to _"was that normal for a task like this?"_. The client `await`ed that
...
- 2cd1519: Tell the user what happened when an estimate returns no forecast.
...
- 225ec74: Show the hook-less install notice even when an estimate returns no forecast.
...
- fea82e9: Tell a hook-less Claude Code install that nothing is submitting its completed runs, and how to fix it.
...
- 0884fe8: feat: record outcomes for out-of-domain (void) estimates too
...
- 10bed79: Attribute an actual to the run that produced it, via an opaque provenance tag.
  ...
  default tag and then _delete_ the row that held the real one — losing it silently. It now
...
- b082316: Close the forecast→actual cost loop for the human, and make the tier/limits legible.
