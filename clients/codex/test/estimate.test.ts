// Ported from clients/claude-code/test/estimate.test.ts. The only behavioral
// assertion that differs: outbound context.host must be "codex", not
// "claude-code".
...
describe("runEstimate — happy path", () => {
  it("calls the SDK with context.host=\"codex\" and writes a pending entry", async () => {
    ...
    expect(opts.context.host).toBe("codex");
