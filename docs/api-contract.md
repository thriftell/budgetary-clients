...
| `success` | boolean | no | Whether the agent run completed its objective. Used to separate "failed and burned tokens" from "succeeded and burned tokens" in calibration. **Send it only if you measured it** — absent means the outcome was not observed; it does not mean the run failed, and it does not mean it succeeded. See below. |
...
| `censoring` | string | no | Best-effort run-termination category (0099a) — **how the run ended**, from a closed four-value vocabulary. Measured harness- or client-side, never model-supplied. ⚠️ `null` means **unknown**, *not* "ran to completion". See below. |
| `cap_ms` | integer | no | The operative **wall-clock** cap in force for this run, in milliseconds. The cap **mechanism**, not a bound on token spend. `null` when unknown or none. See below. |
| `cap_tokens` | integer | no | The operative **per-run token** cap in force for this run, where one exists. `null` when unknown or none. See below. |

**Outcome observation (0024g).** ⚠️ **`success` is optional, and absent means the outcome was not observed; it does not mean the run failed, and it does not mean it succeeded.** An omitted key and an explicit `null` are the same statement. It is three-valued — succeeded / failed / **unobserved** — and a reader that collapses it to two is making a claim the data does not support, in whichever direction it collapses.

Send it **only if something actually measured it**: a harness reporting its own oracle's verdict, or a person answering for a run they watched. Do **not** derive it from how a session or window closed (that describes the *session*, not the *run* — a session holds many runs, and the one being reported may be hours old), do not derive it from the shape of a transcript, and never ask the model whether its own run succeeded — a model's account of its own work is not a measurement. Same class of fact, and the same rule, as `censoring`: **omitted rather than guessed.**

⚠️ **The value rides the first submission or not at all.** `POST /v1/actuals` is idempotent on `estimate_id` (§8), so only the **first** stored submission's `success` persists; a later call for the same `estimate_id` is accepted (`202`) and its value is **discarded**. There is no update endpoint. An omission is therefore exactly as permanent as a value — which is deliberate: the alternative would let a caller that did not observe the outcome overwrite one that did. If you have a verdict, send it with the counts; if you do not, omit the field and submit the counts anyway. Omitting one unobserved field is always better than dropping the whole submission — the token counts you *did* measure are the contract, and they are worth keeping on their own.

On `GET /v1/ledger`, an entry whose actual carries an unobserved outcome serves `"success": null` inside its `actual` object. Render it as a third state (*not reported*), never as a failure glyph: a truthy check on this field displays "we don't know" as "it failed".
...
**Censoring signal (0099a).** `censoring` / `cap_ms` / `cap_tokens` record **how a run ended, and under what cap** — a low-cardinality **category** plus two plain integers (never a message, never a stop reason as free text, never a transcript — only a vocabulary member and integers). `censoring` is one of exactly four values: `natural` (the run ended on its own; no cap was reached) · `harness_watchdog` (a wall-clock watchdog in the calling harness killed it) · `operative_cap` (a cap inside the agent host fired — max turns, context exhaustion, a declared token budget) · `kill_switch` (a human or an automation deliberately aborted it). `cap_ms` is the wall-clock cap that was in force, in milliseconds; `cap_tokens` is the per-run token cap where one exists. ⚠️ **`cap_ms` records the cap *mechanism*, not a bound on token spend** — milliseconds bound nothing on a token scale, and a run stopped on the clock stopped at whatever token count it happened to have reached. Only `cap_tokens` is a bound on what the run could have spent. All three are **measured harness- or client-side, never model-supplied**: read the host's own termination reason — the same class of fact as the token counts — and **never ask the model whether it was cut off** (a model's account of its own truncation is not a measurement). They are **omitted rather than guessed**: a host that exposes no termination reason sends nothing at all. ⚠️ **Send them on the first and only submission for the run.** `POST /v1/actuals` is idempotent on `estimate_id` (§8), so only the **first** submission's values are stored: a later call for the same `estimate_id` is accepted (`202`) and its `censoring` / caps are **discarded**. A two-phase submit — spend now, termination category once the host reports it — therefore drops the category silently, with a success status and no signal anywhere, which is exactly the loss these fields exist to prevent.

⚠️ **`null` means *unknown*, not *uncensored*.** Every row written before these fields existed, and every call that does not send them, stores `null` — which records that the run's ending **was not observed**, not that the run reached its natural end. There is no backfill and there cannot be one: the fact was never captured. A reader who treats absence as "ran to completion" is making a claim the data does not support — and it is precisely the claim that biases a spend distribution toward its short runs, because the runs a cap stopped are the long ones. Treat the field as three-valued: observed, censored, unknown.

They are **best-effort and fail-closed**, exactly like `trace` and the two count pairs: an unrecognised category (matched **exactly** — no case folding, no trimming, no aliasing, because a normalizing coercion is where a typo becomes a confident wrong category), a boolean, a negative, or an out-of-range integer is **dropped to `null`**, the call still returns `202`, no `422` (the total is the contract, these are a bonus). ⚠️ **Unlike the acceptance and symbol pairs, these three fields are independent and coerce independently** — a malformed `cap_ms` does **not** take `censoring` down with it, because the category is the fact that cannot be reconstructed afterwards and the caps only refine it. They are never echoed back and surface on **no** read endpoint: no `assessment` block, no `scheme_version` change.
...
`actual.success` is `true`, `false`, or **`null`** — and `null` means the outcome was **not observed**, not that the run failed (0024g, above). Treat it as three-valued when rendering: a truthy check turns an unobserved run into a reported failure.
...
```json
{
  ...
  "donate_url": null,
  "engine_fingerprint": "3f2a9c1d4e6b"
}
```
...
| `engine_fingerprint` | string | Opaque 12-character lowercase-hex witness of the engine generation this deployment reports. An **operator equality-check**: compare it against a locally computed value to confirm the deployment is running the generation you expect. Equal fingerprints mean equal *generation labels* — a deployment that pins its label reports the pin, so this is not a build identifier. **No client semantics**: do not parse it, do not branch on it, and do not treat a change in its value as a contract change (§9's opaque-token rule applies). Always present. |

All fields except `mode`/`price` are server-config-driven; of those, the three URLs may be `null`, while `engine_fingerprint` is always present. Clients must treat a `null` `donate_url` as "no donate affordance". See [shareware.md](shareware.md) for the service model.
...
- `estimate_id`, `ledger_entry_id`, and `engine_fingerprint` are **opaque tokens**: store them, compare them for equality, and use them as keys — that is exactly what they are for (`POST /v1/actuals` dedups on `estimate_id` per §8; a `GET /v1/ledger` entry carries `estimate_id` so it can be paired back to the estimate that produced it; `engine_fingerprint` is an operator equality-check per §4.6). What you must not do is look *inside* one: do not parse it, do not read meaning into its format, length, prefix, or any substring, and do not branch on what the value *contains*.
- The **value** an opaque token carries is not itself part of the contract, so meeting a value you have not seen before is never a contract change — every estimate is issued its own `estimate_id`, and `engine_fingerprint` changes whenever the deployed build does. What is stable is the field — its name and its type, per the first bullet — never its contents.
...
| 2026-08-09 | **`success` on `POST /v1/actuals` becomes optional.** It is an OBSERVATION, not a status: absent means the outcome was not observed; it does not mean the run failed, and it does not mean it succeeded. An omitted key and an explicit `null` are the same statement, and both `202`. A required boolean made "I did not measure this" unrepresentable, so a caller without an oracle had to manufacture a verdict — permanently, since the route is idempotent on `estimate_id` (§8) and there is no update endpoint. Explicit `true`/`false` are unchanged and still stored verbatim, so **every existing client keeps working byte-for-byte** — additive per §3 in the request direction. ⚠️ **One read-side widening to note:** `actual.success` on `GET /v1/ledger` is now `boolean \| null` where it was always `boolean`. No field was removed or renamed and no semantics changed for a value that is present, but a consumer that renders this field through a truthy check will display an unobserved run as a **failure** — render the three states distinctly. One column relaxation (`DROP NOT NULL`) + one migration; **no backfill** — existing rows keep their stored value, and the manufactured ones among them are recorded in the spec rather than rewritten. No estimate math change, no `scheme_version` bump. |
| 2026-08-18 | Added `engine_fingerprint` (an opaque 12-character lowercase-hex witness of the engine generation currently serving) to the **`GET /v1/meta` 200 body**. It lets an operator confirm **which engine generation a deployment reports** with one unauthenticated `curl`, where previously only a database read could answer. Equal fingerprints mean equal *generation labels*, not necessarily identical builds — a deployment that pins its label reports the pin. **Opaque by construction**: it carries no names, no mechanism and no semantics, and it is an **equality-check for operators, not a signal for clients** — do not parse it, do not branch on it, and do not treat a change in its value as a contract change (§9's opaque-token rule applies to it exactly as it does to `estimate_id`). ⚠️ The value changes whenever the deployed build changes, so the `/v1/meta` body is **not** a cacheable constant — the endpoint never promised one, and `GET /v1/health` has varied in both values and key set since Tier-3. No existing field changed, no request change, no auth change, no migration, no `scheme_version` bump — additive per §3. |
| 2026-08-19 | **Correction, not a change.** §9's opaque-token rule now states its own full scope and enumerates `engine_fingerprint`. The 2026-08-18 row above, and the §4.6 field row it landed, both already invoked it in that form — *"do not parse it, do not branch on it, and do not treat a change in its value as a contract change (§9's opaque-token rule applies)"* — while §9 itself said only *"`estimate_id` and `ledger_entry_id` are opaque tokens; do not parse them"*: one prohibition where its invocations attributed three, and a rule that did not list the field citing it. §9 now states what was already being attributed to it, and — because an unqualified *"do not branch on it"* would outlaw the idempotent replay §8 documents one section earlier — draws the boundary explicitly: **equality comparison, storage, and use as a key are exactly what these tokens are for**; what is forbidden is inspecting their *contents*. **No behavior change, no field change, no migration** — nothing the server does, sends, or accepts differs from what it did on 2026-08-18. |
